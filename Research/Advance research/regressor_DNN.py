...
### 4. 後續 DNN 資料準備與訓練
    ...
    # 儲存到 JSON 檔案
    file_path = f"Testing_mde_using_loss_Bestcomb_A"
    ...
    # 儲存到 JSON 檔案
    file_path = f"Testing_mde_detailed_using_loss_Bestcomb_A.json"
    with open(file_path, "w") as f:
        json.dump(mde_report_avg, f, indent=4)
    ...
    # 儲存到 JSON 檔案
    file_path = f"Testing_accuracy_using_loss_Bestcomb_A"
    with open(file_path, "w") as f:
