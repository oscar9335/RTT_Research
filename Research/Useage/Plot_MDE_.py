...
for ppp in range(5):
    # 讀取 JSON 數據
    file_path = f"Testing_mde_using_loss_Bestcomb_{ppp}"
    ...
    plt.title("4mcAP 2024_12_14 MDE")
    plt.show()


    # # 讀取 JSON 數據
    # file_path = f"Testing_accuracy_using_loss_Bestcomb_{ppp}"
    # with open(file_path) as file:
    ...
    #     if new_key in label_mapping:  # 確保 key 存在於 mapping
    #         r, c = map(int, label_mapping[new_key].split('-'))
    #         grid[rows - r, c - 1] = value["accuracy"]  # 反轉行索引以正確對應圖表
    #         labels[rows - r, c - 1] = label_mapping[new_key]
    ...
    # plt.title("4mcAP 2024_12_14 Accuracy")
    # plt.show()